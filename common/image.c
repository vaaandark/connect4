...
fb_image *fb_get_sub_image(fb_image *img, int x, int y, int w, int h)
{
	...
	if (img == NULL)
		return NULL;
	if ((x < 0) || (y < 0) || (w < 0) || (h < 0) ||
	    (x + w > img->pixel_w) || (y + h > img->pixel_w))
		return NULL;
	...
	if (ret != NULL) {
		...
		ret->pixel_w = w;
		ret->pixel_h = h;
		if (img->color_type != FB_COLOR_ALPHA_8)
			x *= 4;
...
fb_image *fb_read_jpeg_image(char *file)
{
	...
	image = (fb_image *)fb_new_image(FB_COLOR_RGB_8880, cinfo.output_width,
...
/** read a font image **/
fb_image *fb_read_font_image(const char *text, int pixel_size,
			     fb_font_info *info)
{
	if (face == NULL) {
	...
	if ((text[0] & 0x80) == 0) {
		...
		sinfo.bytes = 3;
		ucs4 = ((text[0] & 0x0F) << 12) | ((text[1] & 0x3F) << 6) |
		       (text[2] & 0x3F);
	} else if ((text[0] & 0xF8) == 0xF0) {
		sinfo.bytes = 4;
		ucs4 = ((text[0] & 0x07) << 18) | ((text[1] & 0x3F) << 12) |
	...
	image = fb_new_image(FB_COLOR_ALPHA_8, slot->bitmap.width,
	...
	memcpy(image->content, slot->bitmap.buffer,
	       slot->bitmap.rows * slot->bitmap.pitch);
