...
void fb_init(char *dev)
{
	...
	printf("framebuffer info: bits_per_pixel=%u,size=(%d,%d),virtual_pos_size=(%d,%d)(%d,%d),line_length=%u,smem_len=%u\n",
	       fb_var.bits_per_pixel, fb_var.xres, fb_var.yres, fb_var.xoffset,
	       fb_var.yoffset, fb_var.xres_virtual, fb_var.yres_virtual,
	       fb_fix.line_length, fb_fix.smem_len);
...
void fb_draw_line(int x1, int y1, int x2, int y2, int color)
{
	...
	int *buf =
		_begin_draw(x_min, y_min, x_max - x_min + 1, y_max - y_min + 1);
	if (x1 == x2) {
...
#pragma omp parallel for simd
		for (int i = 0; i < h; ++i) {
			...
			// }
			memcpy(dst + i * SCREEN_WIDTH,
			       jpg_src + i * image->pixel_w, len);
		}
		return;
...
#pragma omp parallel for
	for (i = 0; i < cnt; ++i) {
		fb_draw_image(font_left_bound[i], font_top_bound[i], imgs[i],
			      color);
